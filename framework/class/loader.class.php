...
class Loader {
	...
	function html($name) {
		global $_W;
		if (isset($this->cache['html'][$name])) {
			return true;
		}
		$file = IA_ROOT . '/html/common/' . $name . '.func.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['html'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid App Function /html/common/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
	...
	function mobile($name) {
		global $_W;
		if (isset($this->cache['mobile'][$name])) {
			return true;
		}
		$file = IA_ROOT . '/mobile/common/' . $name . '.func.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['mobile'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid App Function /mobile/common/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
    /************************************end***************************************/
