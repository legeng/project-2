...
function file_upload($file, $type = 'image', $name = '') {
	...
	if(!in_array(strtolower($extention), $_W['config']['upload'][$type]['extentions'])) {
	...
	if(empty($name) || $name == 'auto') {
		...
		$result['path'] .= $filename;
                ...
                /*****************legen add*******************/
		$result['path'] = "{$settings[$type]['folder']}/";
		mkdirs(ATTACHMENT_ROOT . '/' . $result['path']);
                /********************end**********************/
		//$result['path'] .= $name . '.' .$extention;//old version
		$result['path'] .= $name . '.' .'jpg';//leegen add
	}
