...
if($step == 1) {
	...
	if(!empty($uniacid)) {
		...
		$description = $unidata['description'];
                ...
                $city = $unidata['city'];
                $district = $unidata['district'];
                $industry_parent = $unidata['industry_parent'];
                $industry_child = $unidata['industry_child'];
	} else {
		$name = trim($_GPC['uni_name']);
		$description = trim($_GPC['uni_description']);
                $province = $_GPC['uni_province'];
                $city = $_GPC['uni_city'];
                $district = $_GPC['uni_district'];
                $industry_parent = $_GPC['uni_industry_parent'];
                $industry_child = $_GPC['uni_industry_child'];
		isetcookie('uni_name', '', -10000);
		isetcookie('uni_description', '', -10000);
		isetcookie('uni_province', '', -10000);
		isetcookie('uni_city', '', -10000);
		isetcookie('uni_district', '', -10000);
		isetcookie('uni_industry_parent', '', -10000);
		isetcookie('uni_industry_child', '', -10000);
		isetcookie('uni_wxrenzheng', '', -10000);
	}
	...
	if(empty($uniacid)) {
		if(!empty($_GPC['tag'])) {
			...
			$description = trim($_GPC['description']);
                        ...
                        $industry_parent = $_GPC['industry_parent'];
                        $industry_child = $_GPC['industry_child'];
                        $wxrenzheng = trim($_GPC['wxrenzheng']);
			isetcookie('uni_name', '', -1000);
			isetcookie('uni_description', '', -1000);
			isetcookie('uni_province', -1000);
			isetcookie('uni_city', -1000);
			isetcookie('uni_district', -1000);
			isetcookie('uni_industry_parent', -1000);
			isetcookie('uni_industry_child', -1000);
			isetcookie('uni_wxrenzheng', -1000);
		} else {
			$name = trim($_GPC['name']) ? trim($_GPC['name']) : message('抱歉，名称为必填项请返回填写！');
			$description = trim($_GPC['description']);
                        $province = $_GPC['province'];
			...
			isetcookie('uni_province', $province);
			isetcookie('uni_city', $city);
			isetcookie('uni_district', $district);
			isetcookie('uni_industry_parent', $industry_parent);
			isetcookie('uni_industry_child', $industry_child);
			isetcookie('uni_wxrenzheng', $wxrenzheng);
		}
	}
	...
	if(checksubmit('submit') && $flag == 2) {
		...
		pdo_update('uni_account', $data, array('uniacid' => $uniacid));
