...
$dos = array('post', 'delete', 'display', 'details' , 'check');
...
if ($do == 'delete') {
	...
	@unlink(IA_ROOT . '/attachment/'.$acid.'/qrcode_'.$acid.'.jpg');
	@unlink(IA_ROOT . '/attachment/'.$acid.'/headimg_'.$acid.'.jpg');
        rmdirs(IA_ROOT . '/attachment/'.$acid , true);
...
if($do == 'check'){
    $result = pdo_update('uni_account', array('is_check'=>1), array('uniacid' => $uniacid));
    if(!empty($result)){
        message('公众号审核成功', referer(), 'success');
    }else{
        message('公众号审核失败', referer(), 'error');
    }
}
