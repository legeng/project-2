					...
					<ul class="panel-body list-group ">
						...
						<li class=" row list-group-item" style="line-height:60px;">
							<div class="col-xs-12 col-sm-12 col-md-2 col-lg-1">
								<img <?php  if(file_exists(IA_ROOT . '/attachment/'.$account['acid'].'/headimg_'.$account['acid'].'.jpg')) { ?> src="<?php  echo $_W['attachurl'];?><?php  echo $account['acid'];?>/headimg_<?php  echo $account['acid'];?>.jpg?acid=<?php  echo $account['acid'];?>"<?php  } else if($account['type'] == '1') { ?>src="resource/images/gw-wx.gif"<?php  } else { ?>src="resource/images/gw-yx.png"<?php  } ?> class="" width="50" height="50"  onerror="this.src='resource/images/gw-wx.gif'" />
							</div>
							<div class="col-xs-12 col-sm-12 col-md-3 col-lg-4 item">
                                                            <span class="label label-default"><?php  echo $types[$account['type']]['title'];?></span>&nbsp;<?php  echo $account['name'];?><?php  if($uni['endtime'] > 0) { ?><span style="color:red;"><em><strong>【付费推广】</strong></em>&nbsp;&nbsp;剩&nbsp;<?php  echo round(($uni['endtime']-time())/86400);?>&nbsp;天</span><?php  } ?>
							</div>
							...
							<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 text-right" style="padding-bottom:5px;">
								<?php  if($uni['role'] <> 'operator') { ?>
                                                                <a href="<?php  echo url('account/bind/check', array('acid' => $account['acid'], 'uniacid' => $account['uniacid']))?>" class="btn btn-success <?php echo $uni['is_check']==1 ? 'disabled ' : ''?>" data-toggle="tooltip" data-placement="top" title="<?php echo $uni['is_check']==1 ? '已审核' : '未审核'?>" role="button" style="color:#fff"  onclick="return confirm('确认审核吗,审核之后公众号信息将会在前台展示？');return false;"><?php echo $uni['is_check']==1 ? '已审核' : '未审核'?></a>
