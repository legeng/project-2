...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$source = IA_ROOT . "/mobile/themes/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/mobile/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
	...
	if(!is_file($source)) {
		$source = IA_ROOT . "/mobile/themes/default/{$filename}.html";
		$compile = IA_ROOT . "/data/tpl/mobile/default/{$filename}.tpl.php";
	}
