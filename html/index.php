...
$controllers = array();
$handle = opendir(IA_ROOT . '/html/source/');
if(!empty($handle)) {
...
$init = IA_ROOT . "/html/source/{$controller}/__init.php";
if(is_file($init)) {
	require $init;;
}

$actions = array();
$handle = opendir(IA_ROOT . '/html/source/' . $controller);
if(!empty($handle)) {
	while($dir = readdir($handle)) {
...
$_W['attachurl'] = str_replace('html/','',$_W['attachurl']);
require _forward($controller, $action);

function _forward($c, $a) {
	$file = IA_ROOT . '/html/source/' . $c . '/' . $a . '.ctrl.php';
	return $file;
}
